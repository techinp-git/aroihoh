...
สถานะ: **EP-14 done ครบทุก subtask (US-50…57)** (8 ก.ย. 2026) — พิมพ์สติกเกอร์ → สแกนได้แต้ม → แลกรางวัล → คนขายยืนยัน → รายงาน/กันโกง → แต้มอัตโนมัติจากออเดอร์ → ใช้แต้มเป็นส่วนลด
...
| US-56 ✅ | แต้มอัตโนมัติจากออเดอร์ delivery ตอน `completed` (X แต้ม/บาท) · Flex แจ้งได้แต้ม (ผ่านคิว notifications) | 0.5 วัน |
| US-57 ✅ | ใช้รางวัลเป็นส่วนลดตอนสั่งใน LIFF (`orders.discount` คิด server) · audience rule "แต้ม ≥ N" | 1 วัน |
...
2. **แต้มหมดอายุไหม** — ยังไม่เคาะ · ledger มี type `expire` รองรับไว้แล้ว ถ้าจะทำต้องมี job + แจ้งลูกค้า (เฟส 2)
3. ~~**QR พิมพ์ที่ไหน**~~ — **เคาะแล้ว 8 ก.ย. 2026: สติกเกอร์ใต้ฝากล่อง** · US-51 ทำทั้งแผ่นพิมพ์เอง (A4 5×7 ดวงละ 40mm) และ CSV ส่งโรงพิมพ์
...
## 13. บันทึกตอนลง US-51 (8 ก.ย. 2026)
- เคาะแล้ว: **สติกเกอร์ใต้ฝากล่อง** → ทำทั้ง 2 ทาง — 🖨️ แผ่น A4 กริด 5×7 ดวงละ 40×40mm (พิมพ์เองล็อตเล็ก/ทดสอบ) และ ⬇ CSV `code,human,points,url` + BOM (ส่งโรงพิมพ์ล็อตใหญ่)
- **ลิงก์ใน QR ประกอบที่ server** (`GET /admin/loyalty/batches/:id/codes` คืน `url` มาเลย) เพราะ `liffId` อยู่กับแบรนด์ และหน้า line-config เปิดให้แค่ owner — manager ที่สั่งพิมพ์จะดึงเองไม่ได้
- ⚠️ **ไม่มี LIFF ID = ปุ่มพิมพ์ปฏิเสธและบอกวิธีแก้** (CSV ยอมให้โหลดแต่เตือน) — พิมพ์สติกเกอร์ 500 ใบที่สแกนแล้วเปิดไม่ได้คือความเสียหายจริง
- `qrcode` ในแอดมินโหลดแบบ **dynamic import ตอนกดพิมพ์** เท่านั้น (แยก chunk 10KB gz) เช่นเดียวกับ `jsqr` ของ US-54
- ทดสอบครบวง: สร้างล็อต 12 ใบ → พิมพ์ (12 QR data URL, A4 5 คอลัมน์, หัวกระดาษ "แบรนด์ · ชื่อล็อต · 12 ดวง") → **ถอด QR กลับด้วย jsQR ได้ `https://liff.line.me/<liffId>?e=<code>` ตรงกับที่ US-52 รับ** → CSV 12 แถวมี BOM → เปิดใช้/ยกเลิกล็อตเปลี่ยนสถานะจริง

## 14. บันทึกตอนลง US-55 (8 ก.ย. 2026) — ปิดเฟส 1
- กันโกง 2 ชั้นใน `earn()`: **เพดานสแกนต่อวันต่อลูกค้า** (migration 0017 `brands.loyaltyDailyEarnCap`, null = ค่าเริ่มต้น 5) และ **จำกัดสแกนพลาด 10 ครั้ง/ชม.** (นับใน memory — เป้าหมายคือกันสคริปต์เดารหัส ไม่ใช่ audit ไม่คุ้มเขียน DB ทุกครั้งที่พิมพ์ผิด) · ทั้งคู่ตอบ **429** พร้อม `code: RATE_LIMITED`
- ⚠️ **เกินเพดานแล้วโค้ดต้องไม่ถูกเผา** — เช็คเพดานก่อนเข้า transaction ลูกค้าจึงเอาใบนั้นไปสแกนวันหลังได้ (มี e2e คุม)
- นับ "วัน" ด้วย `bangkokDayStart` ไม่ใช่วัน UTC ไม่งั้นรอบวันตัดตอนบ่ายโมงบ้านเรา
- นับ "สแกนพลาด" เฉพาะ 404 (รหัสใช้ไม่ได้) — ใช้ซ้ำ/เพดานเต็มไม่ใช่การเดารหัส ไม่ควรทำให้ลูกค้าปกติโดนกั้น
- `detectAnomalies` = sliding window ต่อคน คืนช่วงที่หนาแน่นที่สุดคนละ 1 รายการ · เกณฑ์ 4 ใบใน 10 นาที · ทดสอบจริงแล้วจับได้
- ปรับแต้มมือ (owner เท่านั้น): **บังคับใส่เหตุผล** · หักเกินยอด → 422 (เงื่อนไข `pointsBalance >= n` เดียวกับตอนแลก) · ลง ledger type `adjust` + audit log
- รายงานบอก **"แต้มค้างในระบบ"** = ภาระที่ร้านต้องเตรียมจ่ายคืนเป็นของรางวัล — เลขนี้สำคัญกว่ายอดแจกสะสม · ตรวจแล้วตรงกับ earned − redeemed
- หน้ารายงานมีปุ่มโหลดใหม่ เพราะเป็นหน้าที่เปิดค้างไว้เฝ้าดู (กดแท็บเดิมซ้ำไม่ remount = ข้อมูลค้าง)

## 15. บันทึกตอนลง US-56 + US-57 (8 ก.ย. 2026) — ปิด EP-14
- **US-56 ให้แต้มตอนออเดอร์ completed** — migration 0018 `brands.loyaltyBahtPerPoint` (null = **ปิด ต้อง opt-in** เพราะแต้มคือภาระที่ร้านต้องจ่ายคืน ไม่ควรเปิดให้เองโดยเจ้าของไม่รู้)
- ฐานคิดแต้ม = **subtotal − discount** ไม่รวมค่าส่ง (เงินก้อนนั้นไปที่ไรเดอร์) และหักส่วนลดออก (ส่วนลดที่มาจากแต้มไม่ควรวนกลับมาเป็นแต้มอีก) · ปัดลง
- ให้แต้ม **ใน transaction เดียวกับการเปลี่ยนสถานะ** ไม่ใช่ fire-and-forget เหมือน push — "ส่งของแล้วแต้มหาย" คือเรื่องที่ลูกค้าทวงจริง · idempotent ด้วยการเช็ค ledger ที่อ้าง orderId เดิม
- Flex "ได้แต้มแล้ว" ผ่านคิวเดิม (`NotifyKind` เพิ่ม `points_earned`, dedupeKey `points:<orderId>`) · dispatch อ่านแต้มจาก ledger ไม่คำนวณซ้ำ — ตัวเลขที่ลูกค้าเห็นต้องตรงกับที่ลงบัญชี
- **US-57 ใช้แต้มเป็นส่วนลดตอนสั่ง** — LIFF ส่งได้แค่ `loyaltyRewardId` ส่วนยอดส่วนลดคิดจาก DB ฝั่ง server (กติกาเหล็ก #2) · ส่วนลดเกินค่าอาหารถูก cap ไว้ที่ค่าอาหาร ไม่ไปลดค่าส่งของไรเดอร์
- ตัดแต้ม + สร้างใบแลก (status confirmed ผูก orderId) + ledger อยู่ใน **transaction เดียวกับการสร้างออเดอร์** → ออเดอร์สำเร็จแต่แต้มไม่ถูกตัด (หรือกลับกัน) เกิดไม่ได้ · `orders.create` จึงถูกห่อ `$transaction` (เดิมเป็น create เดี่ยว)
- รางวัลชนิด `free_item` **ใช้เป็นส่วนลดตอนสั่งไม่ได้ → 400** ต้องไปรับที่ร้าน (คนขายสแกนคูปอง US-54)
- audience rule `points_min` (แต้ม ≥ N) ใช้ยิง broadcast ชวนคนที่มีแต้มค้างมาแลกได้
- ✅ ทดสอบจริง: ลูกค้ามี 10 แต้ม → ใช้ 3 แต้มแลกส่วนลด 20฿ (ยอด 75 → 55฿) → ร้านกดส่งสำเร็จ → ได้อีก 2 แต้ม (จ่ายจริง 40฿ ÷ 20) → เหลือ 9 แต้ม · **ตรวจ invariant ทั้งฐานข้อมูล: ไม่มีลูกค้าคนไหนที่ `pointsBalance` ไม่ตรงกับผลรวม ledger**
