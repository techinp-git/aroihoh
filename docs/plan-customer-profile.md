...
สถานะ: **done ครบ EP-15** (US-58/59/60, 8 ก.ย. 2026) — คู่กับ `plan-loyalty-qr.md` (EP-14) · โปรไฟล์เป็น "บ้าน" ของลูกค้า: ที่อยู่หลายหมุด + แต้ม + รางวัล + ตั้งค่า
...
## 12. บันทึกตอนลง US-60 (8 ก.ย. 2026)
- ลิงก์ privacy policy ใช้ env `VITE_PRIVACY_URL` — **ไม่ตั้ง = ไม่โชว์ลิงก์** เพราะเอกสาร PDPA pack ยังไม่มี (blocker เดิม) พาลูกค้าไปหน้า 404 แย่กว่าไม่มีลิงก์ · เขียน PDPA pack เสร็จเมื่อไหร่แค่ตั้ง env แล้ว build ใหม่
- พิสูจน์บน DB จริง: ตั้ง `ENCRYPTION_KEY` → `phoneEnc` = `v1:iv:tag:ct` ขณะที่ API คืนแค่ 4 ตัวท้าย · คีย์หาย → `hasPhone:false` ไม่ทำ endpoint พัง
- หน้า admin แยกสมุดที่อยู่ออกจาก snapshot ของออเดอร์ (โชว์ snapshot 5 ล่าสุด + นับที่เหลือ)
- **EP-15 ครบทุก subtask** — งานที่ยังเปิดของโปรไฟล์คือการ์ดแต้ม ซึ่งรอ EP-14 (US-50)
