...
- ลงเพิ่ม: `broadcasts` (US-31 — preview reach/create+enqueue/list/detail · pure `segment.ts` resolveAudience หัก opt-out + tag segment · จอง message_logs(queued, dedupeKey) กันส่งซ้ำ · owner/manager เท่านั้น · ยิง LINE จริงรอ SETUP-1 = BullMQ worker) · customers เพิ่ม PATCH /:id/opt-out (PDPA)
...
- **admin UI ครบ 8 หน้า** (feature-complete): Dashboard (สรุป+date picker เลือกวัน US-13), Orders (สถานะ/ยกเลิก/COD/realtime/⬇CSV export), Chat, **ส่งข่าวสาร/Broadcast** (US-31 — compose+เลือก tag segment+preview reach+ประวัติ, ซ่อนจาก staff), Menu (**CRUD เต็ม**: เพิ่ม/แก้/ลบ+หมวด+รูป US-14 · GET /admin/menu/categories), Customers, Users&RBAC(owner), Settings · US-13/14 done · US-19 CSV แล้ว (margin/breakeven ยัง — รอ field ต้นทุนต่อเมนู)
