...
- US-37 จอครัว KDS (EP-12): `GET /admin/kitchen/orders` (ไม่รับ brandId = รวมทุกแบรนด์ที่ admin มีสิทธิ์, active pending/confirmed/preparing/ready, เรียงเก่าสุดก่อน + brandName) · SSE `GET /admin/kitchen/stream?token=` filter ตาม admin.brandIds · admin หน้า "ครัว (KDS)" การ์ด grid: BrandChip สีแบรนด์ + ขอบสีตามสถานะ + ปุ่มดันสถานะ (pending→รับออเดอร์ / confirmed→เริ่มทำ / preparing→จัดเสร็จ / ready→ไรเดอร์รับแล้ว) + beep/flash realtime · reuse changeStatus(brandId ต่อออเดอร์) · US-42/43 จะ hook พิมพ์ที่ transition
