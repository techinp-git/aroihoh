...
- ลงแล้ว: `auth` (US-01 LIFF ID token→JWT), `menu` (US-14 CRUD + public list), `delivery` (US-15 zone check radius + fee flat/tiered/per_km), `health`
- pure domain (unit-tested): `delivery/geo.ts` (haversine, radius), `delivery/fee.ts` (computeDeliveryFee), `orders/pricing.ts` (computeOrderPricing)
- ยังไม่ลง: `line` (webhook/flex/push), `orders` (สร้างออเดอร์เต็ม), `payments`, `notifications` (BullMQ), `admin` (auth จริง)
