...
- ⚠️ **LIFF ยังเปิดใช้ไม่ได้จนกว่าจะ build static ใหม่พร้อม `VITE_LIFF_ID`** — build ที่วางอยู่ `/var/www/aroihoh-liff` ไม่มี LIFF ID ฝัง → ตกไปเส้น `dev-login` ซึ่ง prod ปิด (403) ลูกค้าเข้าไม่ได้เลย · เช็คก่อน deploy เสมอ: `grep -c "<liffId>" /var/www/aroihoh-liff/assets/*.js`
